...
def Load_Dataloader(train_path_list: list, tf, batch_size: int):
    train_data = TrainingDataset(train_path_list, tf)
    train_dataloader = DataLoader(train_data,batch_size=batch_size)
    return train_dataloader

def overall_generator_pass(generator: rGAN.Generator, discriminator: rGAN.Discriminator, img: list[torch.Tensor], gt: torch.Tensor, valid: torch.Tensor) -> (list[torch.Tensor], torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor):
    # print(type(generator), type(img), type(gt), type(valid), ' ++++++')
    # print(generaor.device)
    ...
    imgs = roll_axis(imgs)
    loss= msssim+f1
    g_loss = adversarial_loss(discriminator(recon_batch),valid.cuda()) + loss

    
    # print(type(loss), type(g_loss), type(msssim), type(recon_batch))
    return imgs, g_loss, recon_batch, loss, msssim

def overall_discriminator_pass(discriminator: rGAN.Discriminator, recon_batch: torch.Tensor, gt: torch.Tensor, valid: torch.Tensor, fake: torch.Tensor) -> torch.Tensor:
    real_loss = adversarial_loss(discriminator(gt), valid.cuda())
    fake_loss = adversarial_loss(discriminator(recon_batch.detach()), fake.cuda())
...
def main(k_shots, num_tasks, adam_betas, gen_lr, dis_lr, total_epochs, model_folder_path):
    ...
    # frame_path = '/g/data/cp23/lw4988/few-shot-anomaly-detection/data/'
    frame_path = 'C:/Users/liyun/Desktop/Videos_pic_new/'
        ...
        for _, epoch_of_tasks in enumerate(train_dataloader):
            ...
            for tidx, task in enumerate(epoch_of_tasks):
                ...
                for kidx, frame_sequence in enumerate(task[:k_shots]):
                    ...
                    print ('Epoch [{}/{}], Step [{}/{}], Reconstruction_Loss: {:.4f}, G_Loss: {:.4f}, D_loss: {:.4f},  msssim:{:.4f} '.format(epoch+1, total_epochs, tidx+1, num_tasks, loss.item(), g_loss.item(), d_loss.item(), msssim.item()))
    ...
    gen_path = os.path.join(model_folder_path, str.format("Generator_Final_test.pt"))
    dis_path = os.path.join(model_folder_path, str.format("Discriminator_Final_test.pt"))
    torch.save(generator.state_dict(), gen_path)
    torch.save(discriminator.state_dict(), dis_path)
